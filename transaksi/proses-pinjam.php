...
if (isset($_POST['btnPinjam'])) {
    ...
    if (cekPinjam($kon, $id_anggota) && $stok>0) {
        ...
        $queryp=mysqli_query($kon, "SELECT id_pinjam FROM peminjaman WHERE tgl_pinjam = '$tgl_pinjam' AND id_anggota='$id_anggota' 
                                                                                                      AND tgl_jatuh_tempo='$tgl_jatuh_tempo' 
                                                                                                      AND id_petugas=$id_petugas;");
        $wd=mysqli_fetch_assoc($queryp);
        $idp=$wd['id_pinjam'];
        $sql1=mysqli_query($kon, "INSERT INTO detail_pinjam(id_pinjam, id_buku)
                                  VALUES ('$idp', '$id_buku')");
